...
def test_empty_user_returns_no_snapshot_flag(patch_crud) -> None:
    ...
    assert len(result.capital_chart) == CHART_MONTHS
    # Без фактов ряд actual обнулён (None), план тоже накопительный ноль.
...
def test_happy_path_aggregates_closed_and_current_plan(patch_crud) -> None:
    ...
    # График: 12 точек, ряд actual определён для закрытых месяцев.
    assert len(result.capital_chart) == CHART_MONTHS
